...
def load_data(config):
    ...
    # Initialize dataset
    train_data = CelebA(root='data', split="train", target_type="attr", transform=transforms)
    val_data = CelebA(root='data', split="valid", target_type="attr", transform=transforms)
    test_data = CelebA(root='data', split="test", target_type="attr", transform=transforms)
...
def main(config):
    ...
    for i in range(5):
        plt.imsave(f"stable_sample_{i}.png", samples[i,0,:,:].cpu(), cmap="gray")
